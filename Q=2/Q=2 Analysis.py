...
if __name__ == '__main__':
    ...
    posDir = {}
    error = []
    dirlist = os.listdir('PolTwop_Q=2')
    #print(dirlist)
    filesByPosition = hash(dirlist)

    for pos in positions:
        stopPointFile = len(filesByPosition[pos])
        dir = DirectoryToDictionary('PolTwop_Q=2',filesByPosition[pos],stopPointTime,stopPointFile)
    ...
    plt.title('Averages of Q=2 Energy')

    plt.plot(posDir['+2_+0_-3'].values(),marker='o',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['-2_+0_-3'].values(),marker='*',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['+0_+2_-3'].values(),marker='^',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['+0_-2_-3'].values(),marker='v',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['+2_+0_+3'].values(),marker='o',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['-2_+0_+3'].values(),marker='*',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['+0_+2_+3'].values(),marker='^',markersize='4',label='+1_+0_-3')
    plt.plot(posDir['+0_-2_+3'].values(),marker='v',markersize='4',label='+1_+0_-3')
    ...
    plt.errorbar(xr,posDir['+2_+0_-3'].values(),yerr=list(error[0].values()))
    plt.errorbar(xr,posDir['-2_+0_-3'].values(),yerr=list(error[1].values()))
    plt.errorbar(xr,posDir['+0_+2_-3'].values(),yerr=list(error[2].values()))
    plt.errorbar(xr,posDir['+0_-2_-3'].values(),yerr=list(error[3].values()))
    plt.errorbar(xr,posDir['+2_+0_+3'].values(),yerr=list(error[4].values()))
    plt.errorbar(xr,posDir['-2_+0_+3'].values(),yerr=list(error[5].values()))
    plt.errorbar(xr,posDir['+0_+2_+3'].values(),yerr=list(error[6].values()))
    plt.errorbar(xr,posDir['+0_-2_+3'].values(),yerr=list(error[7].values()))
